...
export function isPreferredLanguage(value) {
    return typeof value === "string" && languages.includes(value);
}
export function validateDiagnosticAIResult(value) {
    if (!isRecord(value))
...
function language(value) {
    if (!isPreferredLanguage(value))
        throw new Error("invalid_analysisLanguage");
    return value;
}
function confidence(value) {
    if (typeof value !== "number" || value < 0 || value > 1)
        throw new Error("invalid_confidenceLevel");
    return value;
}
function checkArray(value) {
    if (!Array.isArray(value))
        throw new Error("invalid_recommendedChecks");
    return value.map((item) => {
        if (!isRecord(item))
            throw new Error("invalid_recommendedChecks");
        const safetyLevel = item.safetyLevel;
        if (!["low", "medium", "high"].includes(String(safetyLevel)))
            throw new Error("invalid_safetyLevel");
        if (typeof item.order !== "number")
            throw new Error("invalid_check_order");
        return {
...
function measurementArray(value) {
    if (!Array.isArray(value))
        throw new Error("invalid_expectedMeasurements");
    return value.map((item) => {
        if (!isRecord(item))
            throw new Error("invalid_expectedMeasurements");
        return {
...
function sourceArray(value) {
    if (!Array.isArray(value))
        throw new Error("invalid_sourceReferences");
    return value.map((item) => {
        if (!isRecord(item))
            throw new Error("invalid_sourceReferences");
